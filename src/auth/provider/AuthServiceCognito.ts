...
const refreshAccessToken = async (refreshTokenJwt: string): Promise<AuthenticationData> => {
  const user = getCurrentUser();
  ...
  return await new Promise<AuthenticationData>((resolve, reject) => {
    ...
    if (user == null) {
      reject(new AuthenticationError('Failed to refresh access token: User is null...'));
    }
    ...
    user?.refreshSession(cognitoRefreshToken, (err, authSession) => {
      if (err != null) {
        reject(new AuthenticationError('Refreshing of access token failed!'));
      }
